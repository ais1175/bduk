...
if not bduk.is_server then
    ...
    --- Key list copied from BDTK to keep BDUK standalone <3
    ...
    local function create_dui(location_id)
        ...
        local dui_url = "https://cfx-nui-bduk/ui/index.html"
    ...
    RegisterNetEvent("bduk:cl:sync_dui_data", function(id, updated_data)
        if not dui_locations[id] then return end
            ...
            for _, location in pairs(dui_locations) do
                if location.is_destroyed then
                    ...
                    if location.can_access then
                        should_show = location.can_access()
                    end
